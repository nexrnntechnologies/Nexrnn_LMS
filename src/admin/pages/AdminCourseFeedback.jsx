...
export default function AdminCourseFeedback() {
  ...
  return <div className="max-w-6xl mx-auto px-8 py-10">
    <div className="flex flex-wrap items-start justify-between gap-4 mb-6"><div><p className="text-xs font-bold tracking-[0.2em] text-slate-400 uppercase mb-2">Learner voice</p><h1 className="text-2xl font-extrabold text-slate-900">Feedback</h1><p className="text-sm text-slate-500 mt-1">Create, view, edit and delete learner feedback. Admin-added feedback is shown on the website under the selected learner's name.</p></div><div className="flex flex-wrap items-center gap-3"><AdminExportButtons title="learner-feedback" rows={visible} columns={[{ label: "Course / Workshop", value: (item) => item.courseTitle || item.course_id }, { label: "Learner", key: "learner_name" }, { label: "User ID", key: "user_id" }, { label: "Rating", key: "stars" }, { label: "Feedback", key: "comment" }, { label: "Date", value: (item) => item.created_at ? new Date(item.created_at).toLocaleString("en-IN") : "" }]} /><button onClick={openCreate} disabled={!users.length || !courses.length} className="inline-flex items-center gap-1.5 text-sm font-semibold text-white px-4 py-2.5 rounded-md hover:opacity-90 disabled:opacity-50" style={{ backgroundColor: BLUE }}><Plus size={15} /> Add Feedback</button></div></div>
