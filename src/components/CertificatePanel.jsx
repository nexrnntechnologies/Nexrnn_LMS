  ...
  return <div>
    ...
    <div className="rounded-xl p-2 sm:p-4 shadow-sm" style={{ backgroundColor: "#fff", border: `2px solid ${NAVY}` }}>
      <div className="rounded-lg border-[2px] border-dashed border-slate-200 px-4 sm:px-8 py-10 sm:py-14 text-center">
        ...
        <p className="text-lg sm:text-xl font-bold mt-2" style={{ color: BLUE }}>{course.title} Course</p>
    ...
    <div className="flex flex-wrap items-center gap-3 mt-5"><button onClick={() => downloadCertificatePdf({ studentName, courseName: `${course.title} Course`, registrationId, issuedDate })} className="flex items-center gap-2 text-sm font-bold text-white px-4 py-2.5 rounded-md hover:opacity-90" style={{ backgroundColor: BLUE }}><Download size={15} /> Download PDF</button><button onClick={handleShare} className="flex items-center gap-2 text-sm font-bold px-4 py-2.5 rounded-md border border-slate-300 text-slate-700 hover:bg-slate-50"><Share2 size={15} /> Share Link</button><button onClick={handleShare} className="flex items-center gap-2 text-sm font-semibold text-slate-500 hover:text-slate-800"><LinkIcon size={14} /> Copy link</button></div>{shareMessage && <p className="text-sm text-slate-500 mt-3">{shareMessage}</p>}
