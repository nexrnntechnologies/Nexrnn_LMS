...
export default function CoursePlayerView({ courseType = "course" }) {
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-[calc(100vh-64px)] lg:h-[calc(100vh-64px)]">
      <aside className="w-full lg:w-80 border-b lg:border-b-0 lg:border-r border-slate-200 bg-white shrink-0 lg:overflow-y-auto">
        <div className="flex items-center justify-between gap-3 px-4 sm:px-5 pt-4 lg:pt-5">
          <button onClick={() => navigate(isPreview ? `${catalogPath}/${courseId}` : "/dashboard")} className="flex items-center gap-1.5 text-sm font-semibold text-slate-500 hover:text-slate-800"><ChevronLeft size={15} /> {isPreview ? `Back to ${itemLabel}` : "Go to Dashboard"}</button>
          <button type="button" onClick={() => setCurriculumOpen((current) => !current)} className="lg:hidden inline-flex items-center gap-1.5 rounded-md border border-slate-200 px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50"><span>{curriculumOpen ? "Hide" : "Show"} content</span><ChevronRight size={14} className={`transition-transform ${curriculumOpen ? "rotate-90" : ""}`} /></button>
        </div>
        <div className="px-4 sm:px-5 pt-4"><h2 className="font-extrabold text-slate-900 leading-snug">{course.title}</h2><div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden mt-3 mb-1"><div className="h-full rounded-full" style={{ width: `${pct}%`, backgroundColor: BLUE }} /></div><p className="text-[12px] text-slate-500 mb-4">{isPreview ? "Free preview" : `${pct}% complete`}</p></div>
        <div className="px-4 sm:px-5 pb-4"><div className="rounded-lg p-4" style={{ backgroundColor: NAVY_SOFT }}><p className="text-[13px] text-slate-200 mb-3">Join the conversation in the Nexrnn {course.title} — Community.</p><button onClick={() => navigate("/community")} className="w-full text-xs font-bold text-white border border-white/20 rounded-md py-2 flex items-center justify-center gap-1 hover:bg-white/10">GO TO COMMUNITY <ArrowRight size={12} /></button></div></div>
        <nav className={`${curriculumOpen ? "block" : "hidden"} lg:block border-t border-slate-100 max-h-[55vh] lg:max-h-none overflow-y-auto`}>
          <div className="px-4 sm:px-5 py-3 lg:hidden"><p className="text-[11px] font-bold tracking-wide text-slate-400 uppercase">Course content</p><p className="text-xs text-slate-500 mt-1">Choose a lesson to study</p></div>
          {modules.map((module) => <div key={module.id} className="border-b border-slate-100">
            <button onClick={() => toggleModule(module.id)} className="w-full flex items-center justify-between px-4 sm:px-5 py-3 text-left"><span className="text-sm font-bold text-slate-800">{module.title}</span><span className="text-[11px] text-slate-400 font-semibold shrink-0 ml-2">{isPreview ? `${module.lessons.filter((lesson) => lesson.freePreview).length} preview` : `${module.lessons.filter((lesson) => lesson.done).length}/${module.lessons.length}`}</span></button>
            {openModules[module.id] && <div>{module.lessons.map((lesson) => { const LessonType = lesson.type === "video" ? Video : FileText; const active = activeLesson.id === lesson.id; const locked = isPreview && !lesson.freePreview; return <button key={lesson.id} onClick={() => { goToLesson(lesson); setCurriculumOpen(false); }} className={`w-full flex items-center gap-3 px-4 sm:px-5 py-3 text-left border-l-2 ${active ? "bg-slate-50" : "hover:bg-slate-50"}`} style={{ borderColor: active ? BLUE : "transparent" }}>{locked ? <Lock size={15} className="text-slate-300 shrink-0" /> : lesson.done ? <CheckCircle2 size={16} style={{ color: BLUE }} className="shrink-0" /> : <Circle size={16} className="text-slate-300 shrink-0" />}<div className="min-w-0 flex-1"><p className={`text-[13px] font-medium truncate ${locked ? "text-slate-400" : "text-slate-700"}`}>{lesson.title}</p><p className="text-[11px] text-slate-400 flex items-center gap-1 mt-0.5"><LessonType size={11} /> {lesson.type === "video" ? lesson.duration || "VIDEO" : lesson.pdfUrl ? "PDF" : "TEXT"}</p></div><span className={`text-[9px] font-bold px-1.5 py-0.5 rounded shrink-0 ${lesson.freePreview ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-500"}`}>{lesson.freePreview ? "FREE" : "PAID"}</span></button>; })}</div>}
          </div>)}
        </nav>
      </aside>

      <main className="flex-1 min-w-0 min-h-[calc(100vh-64px)] lg:min-h-0 overflow-y-auto bg-white"><div className="border-b border-slate-200 px-4 sm:px-6 lg:px-8 py-4"><p className="text-sm font-semibold text-slate-500 break-words">{activeLesson.title}</p></div><div className="px-4 sm:px-6 lg:px-8 py-6 sm:py-8 max-w-4xl">
