...
export default function ProfileView({ certificateType = null }) {
  ...
  return <div className="max-w-6xl mx-auto px-6 py-10 flex flex-col lg:flex-row gap-8">
    <aside className="w-full md:w-56 shrink-0"><nav className="flex lg:flex-col gap-1 overflow-x-auto">{tabs.map((tab) => { const Icon = tab.icon; const active = activeTab === tab.key; return <button key={tab.key} onClick={() => navigate(tab.path)} className={`flex items-center gap-2 px-3 py-2.5 rounded-md text-sm font-semibold text-left whitespace-nowrap ${active ? "text-white" : "text-slate-600 hover:bg-slate-50"}`} style={active ? { backgroundColor: NAVY } : {}}><Icon size={15} /> {tab.label}</button>; })}</nav></aside>
