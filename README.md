...
- **Public LMS portal** — the landing page plus Course and Workshop catalogs/details/previews are open without login; users sign in or create an account only when enrolling or opening account areas
...
- **Dashboard** (`/dashboard`) — one learner dashboard with upper **My Course** and lower **My Workshop** sections, live progress and one-time ratings
- **Workshops** (`/workshops`) — public workshop catalog/details and authenticated learner Workshop dashboard/player at `/my-workshops`; workshops reuse course enrollment, progress, ratings, content and completion behavior
...
- **My Account** (`/my-account`) — read-only Profile with name, mobile, email, gender and permanent User ID; Password, Notifications and Order History; combined Course and Workshop Certificates at `/dashboard/certificates`
- **Certificates** — a completed Course or Workshop gets a unique immutable Certificate ID, a fixed issue date, persistent verified certificate, PDF download and public share/copy link; titles end in `Course` or `Workshop` respectively
...
- **Feedback** — completed learners can submit a star rating and written feedback once per Course or Workshop; Feedback appears in a left-to-right slider on the landing page, catalog and detail pages, and in Admin → Feedback
...
| **Courses** | Add, edit, delete courses and manage modules/lessons |
| **Workshops** | Separate Workshop CRUD and Workshop → Content module/lesson management; the same admin completion gate controls certificates |
...
| **Issued Certificates** | See every issued Certificate ID, User ID, learner, Course/Workshop type, issue date, and open the public verification link |
...
| **Feedback** | Read completed-learner ratings and written feedback for Courses and Workshops |
...
| Dashboard — My Course + My Workshop (login required) | `/dashboard` |
| Public Workshops | `/workshops` |
| Workshop detail page | `/workshops/digital-marketing-workshop` |
| Workshop player | `/my-workshops/digital-marketing-workshop` |
| A specific enrolled course (player) | `/my-courses/digital-marketing` |
| Combined Course + Workshop Certificates (login required) | `/dashboard/certificates` |
...
| Admin — manage workshops | `/nexrnn/master_nexrnn/admin/workshops` |
| Admin — Workshop content | `/nexrnn/master_nexrnn/admin/workshops/:courseId/content` |
...
| Admin — Feedback | `/nexrnn/master_nexrnn/admin/feedback` |
| Admin — Workshop enrollments | `/nexrnn/master_nexrnn/admin/workshop-enrollments` |

> Note: URLs use lowercase-with-hyphens (`/dashboard`, `/my-account`) instead of spaces,
> since spaces aren't valid in real URLs (browsers turn them into `%20`). This keeps
> `lms.nexrnn.in/dashboard` etc. clean when you deploy.
...
│   └── migration_16.sql    # run sixteenth — course/workshop type, Workshop seed data and typed certificates
└── src/
...
4. **Start the dev server (works immediately, no Supabase needed):**
   ...
   Without a `.env` file, the app runs in **demo mode** using `src/data/mockData.js` — no sign-in required, and `/dashboard` opens directly.
...
2. In your project, go to **SQL Editor → New query**, paste the entire contents of `supabase/schema.sql`, and click **Run**.
   This creates all tables (including typed certificates, modules, lessons, enrollments, ratings, notifications, profiles) and seeds the demo courses plus a starter Workshop.
...
16. **New query** again, paste the entire contents of `supabase/migration_15.sql`, and click **Run**.
   This adds the learner author name and full admin create/edit/delete controls for Feedback.
17. **New query** again, paste the entire contents of `supabase/migration_16.sql`, and click **Run**.
   This adds the `course_type` discriminator, preserves existing Course data/certificates, seeds a starter Workshop for existing projects, and makes the certificate RPC issue `Course` or `Workshop` certificates without changing existing IDs or dates.
18. Go to **Project Settings → API** and copy your **Project URL** and **anon public key**.
19. In the project folder, copy `.env.example` to `.env`:
   ```bash
   cp .env.example .env
   ```
20. Open `.env` and paste in your values:
   ```
   VITE_SUPABASE_URL=https://your-project-ref.supabase.co
   VITE_SUPABASE_ANON_KEY=your-anon-public-key
   ```
21. Restart the dev server (`npm run dev`). The app will now:
   ...
   - Open a unique certificate with the student's profile name and completed course at `/dashboard/certificates`, with download and share-link actions
...
1. Connect Supabase first (steps above, including `migration_2.sql`, `migration_3.sql`, `migration_4.sql`, `migration_5.sql`, `migration_6.sql`, `migration_7.sql`, `migration_8.sql`, `migration_9.sql`, `migration_10.sql`, `migration_11.sql`, `migration_12.sql`, `migration_13.sql`, `migration_14.sql`, `migration_15.sql` and `migration_16.sql`).
...
5. You'll land on `/nexrnn/master_nexrnn/admin` — from there you can:
   ...
   - **Workshops** — add, edit, or delete workshops separately, then manage Workshop content
   ...
   - **Workshop Enrollments** — see Workshop enrollments separately with the same date, contact, payment, status, and View details
   ...
   - **Feedback** — add, edit and delete Course or Workshop feedback on behalf of a selected learner; the selected learner name is shown publicly
   - **Overview** — view website visits and unique visitors captured from public pages
   - **Courses / Workshops** — tick the item-specific **complete / certificate ready** checkbox only after all content is final; leave it unticked for ongoing items
