-- =========================================================
-- Nexrnn LMS — Migration 10
-- Run AFTER migration_9.sql.
-- Adds immutable public User IDs and canonical Certificate IDs.
-- Existing records are backfilled; no user/course data is deleted.
-- =========================================================
...
begin
  if not exists (
    ...
    where conname = 'profiles_user_registration_id_key'
...
returns trigger as $$
begin
  if TG_OP = 'UPDATE' and NEW.user_registration_id is distinct from OLD.user_registration_id then
...
create trigger protect_user_registration_id
  before update on profiles
  for each row execute procedure protect_user_registration_id();
...
begin
  ...
  select nullif(trim(concat_ws(' ', first_name, last_name)), '') into v_student_name
